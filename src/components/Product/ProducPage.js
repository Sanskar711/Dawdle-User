...
const ProductPage = () => {
  ...
  const handleUseCaseDetailsClick = (useCaseId) => {
    navigate(`/product/${productId}/options/useCases/${useCaseId}`);
  };

  const handleWatchVideo = () => {
    if (product.video_link) {
      window.open(product.video_link, '_blank', 'noopener,noreferrer');
    }
  };
    ...
    <>
      ...
      <div className="product-profile">
        ...
        <section className="product-description">
          ...
          <button onClick={handleWatchVideo} disabled={!product.video_link}>
            Watch Product Video
          </button>
        </section>
          ...
          <ul>
            {product.use_cases.length > 0 ? (
              product.use_cases.map((useCase) => (
                <li key={useCase.id} onClick={() => handleUseCaseDetailsClick(useCase.id)}>
